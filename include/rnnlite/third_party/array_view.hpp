...
namespace arv {
    ...
    namespace detail {
        ...
        struct make_indices_impl<First, Step, N, typename std::enable_if<(N > 1 && N % 2 == 0)>::type>
            : detail::make_indices_next<typename detail::make_indices_impl<First, Step, N / 2>::type,
                                        First + N / 2 * Step> {};

        template <size_t First, size_t Step, size_t N>
        struct make_indices_impl<First, Step, N, typename std::enable_if<(N > 1 && N % 2 == 1)>::type>
            : detail::make_indices_next2<typename detail::make_indices_impl<First, Step, N / 2>::type,
                                         First + N / 2 * Step, First + (N - 1) * Step> {};

        template <size_t First, size_t Last, size_t Step = 1>
        struct make_indices_ : detail::make_indices_impl<First, Step, ((Last - First) + (Step - 1)) / Step> {};
     ...
     */
        constexpr array_view() noexcept : length_(0), data_(nullptr) {}
        ...
        /*implicit*/ array_view(std::array<T, N> const& a) noexcept : length_(N), data_(N > 0 ? a.data() : nullptr) {}

        // Note:
        // This constructor can't be constexpr because & operator can't be constexpr.
        template <size_type N>
        /*implicit*/ array_view(T const (&a)[N]) noexcept : length_(N), data_(N > 0 ? std::addressof(a[0]) : nullptr) {
        ...
        /*implicit*/ constexpr array_view(T const* a, size_type const n) noexcept : length_(n), data_(a) {}

        template <class InputIterator, class = typename std::enable_if<std::is_same<
                                           T, typename std::iterator_traits<InputIterator>::value_type>::value>::type>
        explicit array_view(InputIterator start, InputIterator last) :
            length_(std::distance(start, last)),
            data_(start) {}

        array_view(std::initializer_list<T> const& l) : length_(l.size()), data_(std::begin(l)) {}
     ...
     */
        ...
        constexpr array_view<T> slice(check_bound_t, size_type const pos, size_type const length) const {
            return (pos >= length_ || pos + length >= length_) ? throw std::out_of_range("array_view::slice()")
                                                               : array_view<T>{begin() + pos, begin() + pos + length};
        }
        constexpr array_view<T> slice_before(check_bound_t, size_type const pos) const {
            return (pos >= length_) ? throw std::out_of_range("array_view::slice()")
        ...
        constexpr array_view<T> slice(check_bound_t, iterator start, iterator last) const {
            return (start >= end() || last > end() || start > last ||
                    static_cast<size_t>(std::distance(start, last > end() ? end() : last)) >
                        length_ - std::distance(begin(), start))
        ...
        constexpr array_view<T> slice_before(check_bound_t, iterator const pos) const {
            return (pos < begin() || pos > end()) ? throw std::out_of_range("array_view::slice()")
    ...
    namespace detail {
        ...
        inline constexpr bool operator_equal_impl(ArrayL const& lhs, size_t const lhs_size, ArrayR const& rhs,
                                                  size_t const rhs_size, IterL const litr, IterR const ritr) {
    ...
    inline constexpr bool operator==(array_view<T1> const& lhs, array_view<T2> const& rhs) {
    ...
    inline constexpr bool operator==(array_view<T> const& lhs, Array const& rhs) {
        return detail::operator_equal_impl(lhs, lhs.length(), rhs, rhs.size());
    }

    template <class T1, class T2, size_t N>
    inline constexpr bool operator==(array_view<T1> const& lhs, T2 const (&rhs)[N]) {
    ...
    inline constexpr bool operator==(Array const& lhs, array_view<T> const& rhs) {
        return rhs == lhs;
    }
    ...
    inline constexpr auto make_view(Array const& a) -> array_view<typename Array::value_type> {
        return {a};
    }

    template <class T, size_t N>
    inline constexpr array_view<T> make_view(T const (&a)[N]) {
        return {a};
    }

    template <class T>
    inline constexpr array_view<T> make_view(T const* p, typename array_view<T>::size_type const n) {
    ...
    inline constexpr Result make_view(InputIterator begin, InputIterator end) {
        return Result{begin, end};
    }

    template <class T>
    inline constexpr array_view<T> make_view(std::initializer_list<T> const& l) {
        return {l};
    }
    // }}}
